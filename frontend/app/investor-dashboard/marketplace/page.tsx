...
function ListingCard({
  ...
  onPurchased: (orderId: string) => void;
  onBidPlaced: () => void;
}) {
  ...
  return (
    <>
      <article className="overflow-hidden rounded-3xl border border-border bg-card p-6">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <h2 className="truncate text-xl font-semibold text-foreground">{order.campaign?.title || 'Token listing'}</h2>
            <p className="mt-2 text-sm text-muted-foreground">
              {order.campaign?.category || 'Uncategorized'} · seller {shortAddress(order.seller_wallet_address)}
            </p>
            {!order.seller_wallet_address && (
              <p className="mt-2 text-xs text-amber-600 dark:text-amber-400">
                Seller must link their wallet before this order can be purchased on-chain.
              </p>
            )}
          </div>
        ...
        <div className="mt-5 grid grid-cols-3 gap-3">
          <div className="rounded-2xl border border-border bg-background/80 p-3">
            ...
            <div className="mt-1 font-semibold text-foreground">{quantity}</div>
          </div>
          <div className="rounded-2xl border border-border bg-background/80 p-3">
            <div className="text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Each</div>
            <div className="mt-1 font-semibold text-foreground">{price} ETH</div>
          </div>
          <div className="rounded-2xl border border-border bg-background/80 p-3">
            <div className="text-[10px] uppercase tracking-[0.14em] text-muted-foreground">Total</div>
            <div className="mt-1 font-semibold text-foreground">{(quantity * price).toFixed(6)} ETH</div>
          </div>
        </div>

        <div className="mt-5 grid gap-2 sm:grid-cols-[1fr_auto_auto] sm:items-start">
          <BuyTokenButton order={order} onPurchased={onPurchased} />
          ...
          <button
            ...
            style={{
              borderColor: 'color-mix(in srgb, var(--ring) 40%, transparent)',
              color: 'var(--ring)',
          ...
          <Link
            href={`/investor-dashboard/marketplace/${order.campaign_id}`}
            className="inline-flex items-center justify-center gap-2 rounded-2xl border border-border px-4 py-2.5 text-sm font-semibold text-muted-foreground transition hover:border-ring"
          >
    ...
    <div className="flex flex-col gap-6">
      <div className="space-y-3">
        ...
        <p className="max-w-2xl text-sm text-muted-foreground">
          Browse active sell orders. Buy instantly at the listed price, or place a bid with your own offer — the seller will choose.
        </p>
      </div>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="text-sm text-muted-foreground">{filteredOrders.length} listing{filteredOrders.length === 1 ? '' : 's'} available</div>
      </div>
        ...
        <div className="grid gap-4 xl:grid-cols-2">
          {filteredOrders.map((order) => (
            <ListingCard
              key={order.id}
              order={order}
              onPurchased={(orderId) => setOrders((current) => current.filter((item) => item.id !== orderId))}
              onBidPlaced={loadOrders}
            />
          ))}
        </div>
      )}
