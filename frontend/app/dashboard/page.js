...
import {
  ...
  UserCircle, Clock, CheckCircle, AlertCircle,
} from "lucide-react";
...
export default function DashboardPage() {
  ...
  const stats = {
    totalGoal: campaigns.reduce((s, c) => s + parseFloat(c.funding_goal ?? "0"), 0),
          ...
          <div className="flex items-center space-x-4">
            <div className="bg-[#1a2030] border border-white/5 rounded-xl px-4 py-2">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-white text-sm font-medium">Business Owner</span>
              </div>
            </div>
          </div>
        </div>
              ...
              {tabs.map((tab) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-2 px-5 py-3 text-sm font-semibold rounded-t-xl border-b-2 transition duration-200 whitespace-nowrap ${activeTab === tab.id
              ...
              <div>
                ...
                {!loading && campaigns.length === 0 && (
                  <div className="text-center py-16">
                    ...
                    <p className="text-gray-400 text-sm">You haven't created any campaigns yet.</p>
                    <Link href="/create-campaign" className="mt-3 inline-block text-[#a78bfa] text-sm hover:underline">
                    ...
                    return (
                      <div key={campaign.id} className="bg-[#0d1117] rounded-2xl p-6 border border-white/5">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                          {[
                            { label: "Goal", value: `${goal.toFixed(3)} ETH`, accent: false },
                            { label: "Category", value: campaign.category ?? "—", accent: false },
                            { label: "Days Left", value: daysLeft > 0 ? daysLeft : "Ended", accent: false },
                            { label: "Price/Token", value: `${parseFloat(campaign.price_per_token ?? "0").toFixed(6)} ETH`, accent: true },
                          ].map((item) => (
                            <div key={item.label}>
                              <p className="text-xs text-gray-400 mb-1">{item.label}</p>
                              <p className={`font-bold text-sm capitalize ${item.accent ? "text-[#a78bfa]" : "text-white"}`}>
                                {item.value}
                              </p>
                            </div>
                          ))}
                        ...
                        <div className="w-full bg-white/10 rounded-full h-1.5 mb-4">
                          <div className="bg-[#6f42c1] h-1.5 rounded-full" style={{ width: "0%" }} />
                        </div>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-white/10">
                          {["Campaign", "Goal (ETH)", "Category", "Days Left", "Status", "Actions"].map((h) => (
                          ...
                          return (
                            <tr key={campaign.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                              ...
                              <td className="py-4 px-4 text-gray-300">
                                {parseFloat(campaign.funding_goal ?? "0").toFixed(3)}
                              </td>
                              ...
                              <td className="py-4 px-4">
                                <span className={`px-2 py-1 rounded-full text-xs font-semibold ${daysLeft > 0
                                    ? "bg-[#28a745]/20 text-[#28a745]"
                                    : "bg-white/10 text-gray-400"
                                  }`}>
              ...
              <div>
                ...
                <div className="bg-[#0d1117] rounded-2xl p-6 border border-white/5 mb-6">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-sm font-bold text-[#a78bfa]">{stats.profileCompletion}%</span>
                  </div>
                  ...
                  <p className="text-xs text-gray-500 mt-2">
                    Complete your profile to earn trust badges visible to investors
                  </p>
                </div>
                ...
                {profile ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {[
                      ...
                      value ? (
                        <div key={label} className="bg-[#0d1117] rounded-xl p-4 border border-white/5">
                    ...
                    <div className="md:col-span-2 bg-[#0d1117] rounded-xl p-4 border border-white/5">
                      <p className="text-xs text-gray-500 mb-3">Document Verification Status</p>
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        {[
                          { label: "National ID", done: !!profile.national_id_cid },
                          { label: "Selfie", done: !!profile.selfie_cid },
                          { label: "Proof of Address", done: !!profile.proof_of_address_cid },
                          { label: "Business Reg.", done: !!profile.business_reg_cid },
                          { label: "Tax Cert.", done: !!profile.tax_cert_cid },
                          { label: "Bank Statement", done: !!profile.bank_statement_cid },
                        ].map(({ label, done }) => (
