...
//newer contract contains no kyc signing verification on chain , it should be verified before interacting with smart contract 
describe("EquitySecondaryMarketplace", function () {
  ...
  beforeEach(async function () {
    [owner, feeTreasury, campaignManager,
      // kycSigner,
  ...
  describe("Basic Swap Test (Tokens for ETH)", function () {
    it("Should successfully execute an atomic swap between buyer and seller", async function () {
      ...
      const tradeId = ethers.id("supabase-order-uuid-12345");
      //const nonce = 1;
      ...
      const tx = await marketplace.connect(buyer).settleTrade(
        tradeId,
        tokenAddress,
        seller.address,
        tokenAmount,
      ...
      const isSettled = await marketplace.settledTrades(tradeId);
      expect(isSettled).to.be.true;
    });
  });
  it("Should reject settling the same tradeId twice", async function () {
      const tokenAddress = await token.getAddress();
      const tokenAmount = ethers.parseEther("50");
      const pricePerToken = ethers.parseEther("0.01");
      const totalPrice = (tokenAmount * pricePerToken) / ethers.parseEther("1");
      const tradeId = ethers.id("duplicate-trade-id");

      // First settlement succeeds
      await marketplace.connect(buyer).settleTrade(
        tradeId,
        tokenAddress,
        seller.address,
        tokenAmount,
      ...
      // Second attempt with identical tradeId must revert
      await expect(
        marketplace.connect(buyer).settleTrade(
          tradeId,
          tokenAddress,
          seller.address,
          tokenAmount,
